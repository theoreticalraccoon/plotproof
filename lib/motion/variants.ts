...
/** Same as revealUp/revealFade but with an explicit start delay. */
...
// Card hover lift (skill: Hover Micro-interaction / Standard tier). Displacement kept small per
// the skill's "Don't" note so it reads as feedback, not motion.
...
/** Page transition (Next.js App Router route change). */
...
// In-page step change (wizard steps, tab panels), smaller/faster than a full route transition
// since nothing is navigating, just swapping content.
...
/** An element whose value is awaiting an async result. */
...
// Skeleton → content handoff. The skeleton leaves faster than the content arrives, so the two
// overlap and the reader never sees a bare gap where the placeholder used to be.
...
// An inline spinner or status glyph appearing beside a label. Scales in from slightly small so
// it reads as arriving rather than blinking on.
