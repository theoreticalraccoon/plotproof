...
export default function ExplorePage() {
  ...
  return (
    <div className="flex h-screen flex-col">
      <header className="flex flex-wrap items-center gap-x-4 gap-y-1 border-b border-gray-200 px-4 py-3">
        <h1 className="text-lg font-semibold">
          Open Deforestation Map
          <span className="ml-2 text-xs font-normal text-gray-500">by PlotProof</span>
        </h1>
        <button onClick={() => setPanel("about")} className="text-sm text-gray-600 underline">
          About
        </button>
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className={`rounded px-3 py-1.5 text-sm font-medium ${
              reportMode ? "bg-blue-600 text-white" : "bg-gray-800 text-white"
            }`}
          >
      ...
      {reportMode && (
        <div className="bg-blue-50 px-4 py-2 text-sm text-blue-900">
      ...
      <div className="flex min-h-0 flex-1 flex-col md:flex-row">
        ...
        <aside className="w-full overflow-auto border-t border-gray-200 p-4 md:w-96 md:border-l md:border-t-0">
          ...
          {panel === "thanks" && (
            <div className="flex flex-col gap-3">
              <h2 className="font-semibold text-green-700">Thank you — report received.</h2>
  ...
  return (
    <div className="flex flex-col gap-3 text-sm text-gray-700">
      ...
      <p>
        ...
        Deforestation Regulation. It currently shows <strong>{count}</strong> flagged
      ...
      <p className="text-xs text-gray-500">
        ...
        as open data —{" "}
        <a href="/api/public/disputes/export" className="underline">
          download the labels
        </a>
        .
      </p>
  ...
  return (
    <div className="flex flex-col gap-3 text-sm">
      <div>
        ...
        {f.commodity && <p className="text-gray-600">Likely commodity: {f.commodity}</p>}
      </div>

      <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-gray-700">
        <dt className="text-gray-500">Detection confidence</dt>
        <dd>{band}</dd>
        {f.clearingWindow && (
          <>
            <dt className="text-gray-500">Clearing window</dt>
            <dd>
              {f.clearingWindow.earliest} → {f.clearingWindow.latest}
            </dd>
          </>
        )}
        <dt className="text-gray-500">Observed through</dt>
        <dd>{f.observedThrough}</dd>
        <dt className="text-gray-500">Community reports</dt>
        <dd>
      ...
      <div className="flex gap-2">
        <button onClick={onConfirm} className="flex-1 rounded bg-gray-800 px-3 py-2 font-medium text-white">
          This looks right
        </button>
        <button onClick={onDispute} className="flex-1 rounded bg-gray-200 px-3 py-2 font-medium">
          This looks wrong
        </button>
      </div>
